...
const VedioCard = ({props}) => {
  ...
  return (
    ...
    <div className='mt-3 w-72 h-90 shadow-xl rounded-xl '>
      <div>
      <img  className="w-72 h-64 rounded-xl opacity-100 hover:opacity-70 " src={snippet.thumbnails.high.url} alt="thumbnail" />
      </div>
       
      <div>
       
        <div><h2 className={`font-semibold text-${theme}`}>{snippet.title}</h2>
        <p className={`font-bold text-${theme}`}>{snippet.channelTitle}</p>
        {statistics?<p className='grey'>{hours}hr ago  <span>{statistics.viewCount} views</span></p>:null}
